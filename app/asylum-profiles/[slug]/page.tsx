    ...
    <>
      ...
      <PageShell title={profile.h1} breadcrumbs={crumbs}>
        {profile.content.map((p, i) => (
          <p key={i} className="prose-safe mb-4 text-sm text-[#3A4542] leading-relaxed sm:text-base">
            {p}
          </p>
        ))}
        ...
        <h2 className="mt-8 break-words text-lg font-bold text-[#0F2420] sm:text-xl">Related Case Types</h2>
        <ul className="mt-4 space-y-1">
          {profile.relatedCaseTypes.map((s) => {
            const ct = getCaseType(s);
            return (
              <li key={s}>
                <Link href={`/case-types/${s}`} className="inline-flex min-h-[44px] items-center text-[#E07050] hover:underline">
                  {ct?.title ?? s}
                </Link>
              </li>
            );
